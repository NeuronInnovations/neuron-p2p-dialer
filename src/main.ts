...
import { Libp2p, Connection } from 'libp2p/dist/src';
...
declare global {
  interface Window {
    ...
    enterPrivateKeyDiv: HTMLDivElement
    enterRendezvousDiv: HTMLDivElement
    connectToPeerDiv: HTMLDivElement
...
(async function () {
  ...
  window.privateKeyBtn.onclick = async () => {
    ...
    const ethAddr = accountInfo.data.evm_address;

    window.ethPkSpan.innerHTML = ethAddr.slice(0, 6) + "..." + ethAddr.slice(-4);
    ...
    window.enterPrivateKeyDiv.style.visibility = "hidden";
    window.enterRendezvousDiv.style.visibility = "visible";
  ...
  window.goAliveBtn.onclick = async () => {
    window.enterRendezvousDiv.style.visibility = "hidden";
    window.connectToPeerDiv.style.visibility = "visible";

    const urlParams = new URLSearchParams(window.location.search);
    const callee = urlParams.get('d');

    if (callee != null) window.peerInput.value = callee;
  ...
  let dialInfo:{peerId:string,aliveTopic:number} | null;
  let topicInfo:AxiosResponse<any,any> | null;
  let conn:Connection;
  let stream:any;

  function resetConnection(){
    dialInfo=null; topicInfo=null; conn=null;stream=null;
  }

  window.peerInput.onchange  = () => resetConnection();

  // on click send a high5
  window.connectBtn.onclick = async () => {
    try {
      
      if(dialInfo==null) dialInfo = await getPhoneNumber(window.peerInput.value, contractAddress, wl)
      if (dialInfo == null) {
        alert("address not providing service")
        return;
      }  
      if(topicInfo == null) topicInfo = await axios.get(`https://testnet.mirrornode.hedera.com/api/v1/topics/${dialInfo.aliveTopic}/messages?limit=3&order=desc`);
      ...
      if(conn == null) conn = await libp2p.dial(ma);

      //const start = performance.now();
      stream = await conn.newStream(['/nrn-protocol-id/1.0.0']);
      const start = performance.now();
      pipe(
        // Source data
      ...
      alert("The connection is struggling - hold your horses " + e);
      resetConnection()
    }
  }
  ...
  function updateConnList(message?: string) {
    ...
    tr.innerHTML = `<td>High Five Back!</td><td>${message}</td>`;
    window.connlistEl.appendChild(tr)
    //});

    window.connectBtn.disabled = true;
    setTimeout(function () {
      window.connectBtn.disabled = false;
    }, 5000);
    ...
    pipe(
      ...
      async function (source) {
        ...
        for await (const data of source) {
          ...
          updateConnList((end - start).toFixed(2) + " ms")
        }
      }
    )
  }
